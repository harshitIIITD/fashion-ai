...
> {
      ...
      'fashion_design' as Kind, // Using a custom kind for fashion tools
      {
        ...
        properties: {
          inspiration: {
            type: 'string',
            description:
              'Source of inspiration for the color palette (e.g., "sunset", "ocean", "vintage film", "art deco")',
          },
          palette_type: {
            type: 'string',
  ...
  ): ToolInvocation<ColorPaletteParams, ColorPaletteResult> {
    return {
      params,
      getDescription: () => {
  ...
  private async execute(
    ...
    const result: ColorPaletteResult = {
      ...
      returnDisplay: this.formatPaletteResults(
        palette,
        harmony,
        inspirationNotes,
        params,
  ...
  private generateColorPalette(params: ColorPaletteParams): Array<{
    ...
    const inspirationPalettes: Record<
      string,
      Array<{
        name: string;
        hex: string;
    ...
    > = {
      sunset: [
        {
          ...
          usage: 'Accent pieces, accessories',
        },
        {
          name: 'Coral Blush',
          hex: '#FF7F7F',
      ...
      ocean: [
        {
          ...
          usage: 'Light layers, accessories',
        },
        {
          name: 'Azure Blue',
          hex: '#007FFF',
      ...
      forest: [
        ...
        },
        {
          ...
          usage: 'Outerwear, formal pieces',
        },
        {
          name: 'Moss Brown',
          hex: '#8B7355',
          rgb: '139, 115, 85',
          description: 'Earthy brown',
          usage: 'Leather goods, accessories',
        },
        {
          name: 'Bark Brown',
          hex: '#654321',
        ...
        },
        {
          ...
          hex: '#FFFDD0',
          rgb: '255, 253, 208',
          description: 'Natural cream',
          usage: 'Base layers, linings',
        },
      ],
    };
    ...
    let basePalette =
      inspirationPalettes[params.inspiration.toLowerCase()] ||
      this.generateGenericPalette(params);
  ...
  private generateGenericPalette(params: ColorPaletteParams) {
    ...
    return (
      palettes[params.palette_type as keyof typeof palettes] ||
      palettes['Neutral']
    );
  }

  private adjustForSeason(
    palette: Array<{
      name: string;
      hex: string;
      rgb: string;
  ...
  private getHarmonyDescription(paletteType: string): string {
    const descriptions: Record<string, string> = {
      Monochromatic:
        'Colors from the same hue family with varying saturation and brightness',
